...
const normalizeApiBaseUrl = (rawUrl) => {
  ...
  // Remove trailing slashes to keep axios URL joining predictable.
  const trimmed = value.replace(/\/+$/, "");

  // If caller already provided a versioned API base, keep as-is.
  if (/\/api\/v\d+$/i.test(trimmed)) return trimmed;

  // If caller provided only /api, append /v1.
  if (/\/api$/i.test(trimmed)) return `${trimmed}/v1`;

  // If caller provided only the host (or some other path), default to /api/v1.
